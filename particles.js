...
class Particles {
  ...
  explode(x, y, color, t = 20, rscale = 10) {
    for (let i = 0; i < t; i++) {
      this.parts.push(newPart(x, y, color, rscale));
    }
  }
  spiral(x, y, color, t = 20, rscale = 10) {
    for (let i = 0; i < t; i++) {
      this.parts.push(newSpiralPart(x, y, color, rscale));
    }
  }
  sprayUp(x, y, color, t = 1, rscale = 2) {
    for (let i = 0; i < t; i++) {
...
function newPart(x, y, color, rscale) {
  return {
    ...
    update: function () {
      ...
      this.theta += 0.3 - 0.6 * Math.random();
    },
  };
}
...
function newSpiralPart(x, y, color, rscale) {
  return {
    ...
    v: 5,
    color: Math.random() > 0.7 ? "white" : color,
    theta: Math.random() * tau,
    draw: function (ctx) {
      ctx.beginPath();
      let oldW = ctx.lineWidth;
